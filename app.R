...
load("models.RData")
models_list = list(model_fit_arima_boost,model_fit_glmnet,model_fit_rf,model_fit_xgboost,model_fit_prophet)
FORECAST_HORIZON <- 7
  ...
  sidebarLayout(
   ...
   mainPanel(
    ...
    tabPanel("Modeling",fluid=TRUE,
      sidebarLayout(
        sidebarPanel(
          selectInput('modeltype', 'Model', choices = c("ARIMA","GlmNet","Random Forest","XGBoost","Prophet")),
...
server <- function(input, output, session) {
  ...
  df_filtered <- reactive({
    df2 <- subset(df, Date >= input$daterange[1] & Date <= input$daterange[2] & Symbol== input$var1) 
    if (input$var2 == 'Yes') {
      df2 = df2
    }else{
  ...
  output$conditional_plot <- renderPlot({
    ...
    df_filtered() %>% ggplot(aes(x=Date,y = Volume)) + geom_point(colour="#0f52ba") +
      ...
      model_index = reactive({
        model_idx = which(c("ARIMA","GlmNet","Random Forest","XGBoost","Prophet") ==input$modeltype)  
      })    
      ...
      modeltime_tbl_res = reactive({
        ...
        tbl_res = modeltime_model_tbl() %>%
          modeltime_forecast(
            ...
            actual_data = modeltime_train_data(),
            keep_data   = TRUE
          ) 
